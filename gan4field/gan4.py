...
for epoch in range(epochs):
    for cur_batch in dataset:
        ...
        if cur_step%2000==0:
            ...
            save = input("Save(Y/N): ")
            if save.lower() == "y":
                torch.save(disc.state_dict(), "./versions/00_disc_model.pt")
                torch.save(disc_opt.state_dict(), "./versions/00_disc_opt.pt")
                torch.save(gen.state_dict(), "./versions/00_gen_model.pt")
                torch.save(gen_opt.state_dict(), "./versions/00_gen_opt.pt")
        cur_step += 1
